...
'''
    invenio.legacy.bibauthorid.interface
    ------------------------------------
...
from invenio.legacy.bibrecord import record_get_field_value, record_get_field_instances
...
def populate_table_with_limit(table_name, column_names, values, values_tuple_size, \
    ...
    '''
    if max_insert_size is None:
        max_insert_size = cfg['CFG_BIBAUTHORID_SEARCH_ENGINE_MAX_DATACHUNK_PER_INSERT_DB_QUERY']
    column_num = len(column_names)
    summ = 0
    start = 0
...
def export_author(pid):   ### export_person
    ...
    for recid in bibrecs:
        ...
        recid_dict['INSPIRE-record-url'] = ('%s/record/%s' % (cfg['CFG_SITE_URL'], str(recid)),)
    ...
    if canonical_names:
        ...
        author_info['profile_page']['INSPIRE_profile_page'] = ('%s/author/profile/%s' % (cfg['CFG_SITE_URL'], canonical_names[0][0]),)
    else:
        author_info['profile_page']['INSPIRE_profile_page'] = ('%s/author/profile/%s' % (cfg['CFG_SITE_URL'], str(pid)),)
